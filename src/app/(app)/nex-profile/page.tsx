                ...
                <div className="space-y-6">
                    ...
                    <Section icon={<Siren size={18} />} title="NEX Overview">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#A0B1DD' }}>
                    ...
                    <Section icon={<Brain size={18} />} title="Behavioral Patterns">
                        <p className="text-xs mb-3" style={{ color: '#B39A84' }}>
                            ...
                            {BEHAVIOR_OPTIONS.map((behavior) => {
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            background: selected ? 'rgba(199, 90, 90, 0.15)' : 'rgba(10, 30, 84, 0.04)',
                                            border: `1px solid ${selected ? 'rgba(199, 90, 90, 0.4)' : 'rgba(10, 30, 84, 0.1)'}`,
                                            color: selected ? '#C75A5A' : '#A0B1DD',
                                        }}
                                    >
                                        {behavior}
                                    </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {COMMUNICATION_STYLES.map((style) => (
                                <button
                                    ...
                                    style={{
                                        background: form.communicationStyle === style.value ? 'rgba(10, 30, 84, 0.12)' : 'rgba(10, 30, 84, 0.03)',
                                        border: `1px solid ${form.communicationStyle === style.value ? 'rgba(10, 30, 84, 0.4)' : 'rgba(10, 30, 84, 0.08)'}`,
                                    }}
                                >
                                    <p className="text-sm font-medium" style={{ color: form.communicationStyle === style.value ? '#0A1E54' : '#0A1E54' }}>
                                        {style.label}
                                    </p>
                                    <p className="text-xs mt-0.5" style={{ color: '#B39A84' }}>{style.description}</p>
                                </button>
                            ))}
                        </div>
                    </Section>
                            ...
                            {MANIPULATION_OPTIONS.map((tactic) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all duration-200"
                                        style={{
                                            background: selected ? 'rgba(229, 168, 74, 0.15)' : 'rgba(10, 30, 84, 0.04)',
                                            border: `1px solid ${selected ? 'rgba(229, 168, 74, 0.4)' : 'rgba(10, 30, 84, 0.1)'}`,
                                            color: selected ? '#E5A84A' : '#A0B1DD',
                                        }}
                                    >
                                        {tactic}
                                    </button>
                    ...
                    <Section icon={<Zap size={18} />} title="Trigger Patterns">
                        ...
                        <div className="flex gap-2 mb-3">
                            ...
                            <button
                                onClick={addTrigger}
                                className="px-3 py-2 rounded-lg cursor-pointer transition-colors hover:bg-[rgba(10, 30, 84,0.15)]"
                                style={{
                        ...
                        {form.triggerPatterns.length > 0 && (
                            <div className="space-y-2">
                                {form.triggerPatterns.map((trigger) => (
                                    ...
                                    >
                                        <span className="text-sm" style={{ color: '#A0B1DD' }}>{trigger}</span>
                                        <button
                    ...
                    {nexProfile?.aiInsights && (
                        <Section icon={<Sparkles size={18} />} title="AI Behavioral Analysis">
                            ...
                            >
                                <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: '#A0B1DD' }}>
                                    {nexProfile.aiInsights}
                                </p>
                                {nexProfile.dangerLevel !== undefined && (
                                    <div className="mt-4 flex items-center gap-2">
                                        <span className="text-xs font-medium" style={{ color: '#B39A84' }}>Danger Assessment:</span>
                                        ...
                                        <span className="text-xs" style={{ color: '#B39A84' }}>
                                ...
                                {nexProfile.detectedPatterns && nexProfile.detectedPatterns.length > 0 && (
                                    <div className="mt-3">
                                        <span className="text-xs font-medium block mb-1.5" style={{ color: '#B39A84' }}>AI-Detected Patterns:</span>
