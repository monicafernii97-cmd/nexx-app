                ...
                <div className="space-y-6">
                    ...
                    <Section icon={<Siren size={18} />} title="NEX Overview">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-medium mb-1.5 block" style={{ color: '#B8A88A' }}>
                    ...
                    <Section icon={<Brain size={18} />} title="Behavioral Patterns">
                        <p className="text-xs mb-3" style={{ color: '#8A7A60' }}>
                            ...
                            {BEHAVIOR_OPTIONS.map((behavior) => {
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            background: selected ? 'rgba(199, 90, 90, 0.15)' : 'rgba(197, 139, 7, 0.04)',
                                            border: `1px solid ${selected ? 'rgba(199, 90, 90, 0.4)' : 'rgba(197, 139, 7, 0.1)'}`,
                                            color: selected ? '#C75A5A' : '#B8A88A',
                                        }}
                                    >
                                        {behavior}
                                    </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {COMMUNICATION_STYLES.map((style) => (
                                <button
                                    ...
                                    style={{
                                        background: form.communicationStyle === style.value ? 'rgba(197, 139, 7, 0.12)' : 'rgba(197, 139, 7, 0.03)',
                                        border: `1px solid ${form.communicationStyle === style.value ? 'rgba(197, 139, 7, 0.4)' : 'rgba(197, 139, 7, 0.08)'}`,
                                    }}
                                >
                                    <p className="text-sm font-medium" style={{ color: form.communicationStyle === style.value ? '#C58B07' : '#F5EFE0' }}>
                                        {style.label}
                                    </p>
                                    <p className="text-xs mt-0.5" style={{ color: '#8A7A60' }}>{style.description}</p>
                                </button>
                            ))}
                        </div>
                    </Section>
                            ...
                            {MANIPULATION_OPTIONS.map((tactic) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className="px-3 py-1.5 rounded-lg text-xs font-medium cursor-pointer transition-all duration-200"
                                        style={{
                                            background: selected ? 'rgba(229, 168, 74, 0.15)' : 'rgba(197, 139, 7, 0.04)',
                                            border: `1px solid ${selected ? 'rgba(229, 168, 74, 0.4)' : 'rgba(197, 139, 7, 0.1)'}`,
                                            color: selected ? '#E5A84A' : '#B8A88A',
                                        }}
                                    >
                                        {tactic}
                                    </button>
                        ...
                        {form.triggerPatterns.length > 0 && (
                            <div className="space-y-2">
                                {form.triggerPatterns.map((trigger) => (
                                    ...
                                    >
                                        <span className="text-sm" style={{ color: '#B8A88A' }}>{trigger}</span>
                                        <button
                    ...
                    {nexProfile?.aiInsights && (
                        <Section icon={<Sparkles size={18} />} title="AI Behavioral Analysis">
                            ...
                            >
                                <p className="text-sm leading-relaxed whitespace-pre-wrap" style={{ color: '#B8A88A' }}>
                                    {nexProfile.aiInsights}
                                </p>
                                {nexProfile.dangerLevel !== undefined && (
                                    <div className="mt-4 flex items-center gap-2">
                                        <span className="text-xs font-medium" style={{ color: '#8A7A60' }}>Danger Assessment:</span>
