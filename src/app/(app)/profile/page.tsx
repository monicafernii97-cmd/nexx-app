    ...
    if (!user) {
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="animate-pulse-primary text-sm" style={{ color: '#E8DDD3' }}>Loading profile...</div>
            </div>
        );
    }
        ...
        <div className="max-w-3xl mx-auto pb-12">
            ...
            >
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-serif font-bold" style={{ color: '#FFFAF3' }}>
                            My Profile
                        </h1>
                        <p className="text-sm mt-1" style={{ color: '#E8DDD3' }}>
                    ...
                    <button
                        ...
                        style={{
                            background: saved ? 'rgba(90, 158, 111, 0.2)' : 'linear-gradient(135deg, #FFFAF3, #123D7E)',
                            color: saved ? '#5A9E6F' : '#FFFAF3',
                ...
                <div className="space-y-6">
                    ...
                    <Section icon={<Briefcase size={18} />} title="Support Team">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label className="flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-colors hover:bg-[rgba(199, 208, 229,0.04)]"
                                style={{ border: '1px solid rgba(199, 208, 229, 0.1)' }}>
                                <input
                                    type="checkbox"
                                    checked={form.hasAttorney}
                                    onChange={(e) => setForm({ ...form, hasAttorney: e.target.checked })}
                                    className="accent-[#FFFAF3] w-4 h-4"
                                />
                                <div>
                                    <p className="text-sm font-medium" style={{ color: '#FFFAF3' }}>I have an attorney</p>
                                    <p className="text-xs" style={{ color: '#E8DDD3' }}>NEXX will suggest coordinating with counsel</p>
                                </div>
                            </label>
                            <label className="flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-colors hover:bg-[rgba(199, 208, 229,0.04)]"
                                style={{ border: '1px solid rgba(199, 208, 229, 0.1)' }}>
                                ...
                                />
                                <div>
                                    <p className="text-sm font-medium" style={{ color: '#FFFAF3' }}>I have a therapist</p>
                                    <p className="text-xs" style={{ color: '#E8DDD3' }}>NEXX will suggest discussing emotional impacts</p>
                                </div>
                            </label>
                        </div>
                    </Section>
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-1">
                                {TONE_OPTIONS.map((opt) => (
                                    <button
                                        ...
                                        style={{
                                            background: form.tonePreference === opt.value ? 'rgba(199, 208, 229, 0.12)' : 'rgba(199, 208, 229, 0.03)',
                                            border: `1px solid ${form.tonePreference === opt.value ? 'rgba(199, 208, 229, 0.4)' : 'rgba(199, 208, 229, 0.08)'}`,
                                        }}
                                    >
                                        <p className="text-sm font-medium" style={{ color: form.tonePreference === opt.value ? '#FFFAF3' : '#FFFAF3' }}>{opt.label}</p>
                                        <p className="text-xs mt-0.5" style={{ color: '#E8DDD3' }}>{opt.description}</p>
                                    </button>
                                ))}
                            </div>
                        </Field>
                            ...
                            <div className="flex flex-wrap gap-2 mt-1">
                                {EMOTIONAL_STATES.map((s) => (
                                    <button
                                        ...
                                        style={{
                                            background: form.emotionalState === s.value ? 'rgba(199, 208, 229, 0.12)' : 'rgba(199, 208, 229, 0.03)',
                                            border: `1px solid ${form.emotionalState === s.value ? 'rgba(199, 208, 229, 0.4)' : 'rgba(199, 208, 229, 0.08)'}`,
                                        }}
                                    >
                                        <span>{s.emoji}</span>
                                        <span className="text-sm" style={{ color: form.emotionalState === s.value ? '#FFFAF3' : '#C7D0E5' }}>{s.label}</span>
                                    </button>
                                ))}
                            </div>
                        </Field>
...
function Section({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
    return (
        <motion.div
            ...
            style={{
                background: 'rgba(199, 208, 229, 0.02)',
                border: '1px solid rgba(199, 208, 229, 0.1)',
            }}
        >
            <div className="flex items-center gap-2 mb-5">
                <span style={{ color: '#FFFAF3' }}>{icon}</span>
                <h2 className="text-lg font-semibold" style={{ color: '#FFFAF3' }}>{title}</h2>
            </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
    return (
        <div>
            <label className="text-xs font-medium mb-1.5 block" style={{ color: '#C7D0E5' }}>{label}</label>
            {children}
        </div>
    );
}
