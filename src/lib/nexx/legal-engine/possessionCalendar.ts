...
export function resolveRequestedFathersDaySchedule(args: {
  ...
  if (!containsFathersDay(args.userMessage)) return null;
  const explicitYear = args.userMessage.match(/\b(20\d{2})\b/)?.[1];
  if (!explicitYear && !/\bthis year\b/i.test(args.userMessage)) return null;
  const controllingText = args.controllingText.replace(/[’‘]/g, "'");
  if (!/friday preceding father'?s day/i.test(controllingText) ||
      !/monday (?:after|following) father'?s day/i.test(controllingText)) return null;
  const startTime = controllingText.match(/(?:beginning|begins|start(?:ing|s)?)\s+at\s+(\d{1,2}:\d{2}\s*[ap]\.?m\.?)\s+on\s+the\s+friday/i)?.[1];
  const endTime = controllingText.match(/(?:ending|ends)\s+at\s+(\d{1,2}:\d{2}\s*[ap]\.?m\.?)\s+on\s+the\s+monday/i)?.[1];
